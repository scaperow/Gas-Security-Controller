...
namespace GSC
{
    public class LockInput
    {
        ...
        int lResualt;
            ushort iAtom;
            iAtom = Win32.GlobalFindAtom(Win32.SHELL_FALG);
        if (iAtom == 0)
            {
                lResualt = InsertAsmCode();
            }
 
 
            iAtom = Win32.GlobalFindAtom(Win32.ATOM_FLAG);
            if (isLock)
            {
                ...
                if (iAtom == 0)
                {
                    iAtom = Win32.GlobalAddAtom(Win32.ATOM_FLAG);
                }
            }
            else
            {
